...
(function () {
  ...
  var farmLink = "https://blockfarm.club/farm/missions";
  ...
  var wrapBox = document.getElementsByClassName(
    "w-full bg-gray-900 rounded-tr-3xl sahdow-lg p-4 flex flex-col justify-center items-center border"
  );
  ...
  // const redeemBtn = Array.prototype.slice
  //   .call(
  //     document.getElementsByClassName(
  //       "flex-1  text-center inline border-2 font-semibold  py-1 px-1 rounded-md"
  //     )
  //   )
  //   .filter(function (el) {
  ...
  var ww = "ww" + new Date().toLocaleDateString();
  var sn = "sn" + new Date().toLocaleDateString();
  var mn = "mn" + new Date().toLocaleDateString();

  var _worldWaterBtn, sunWaterBtn, moonWarterBtn;
  if (wrapBox && wrapBox[0]) {
    _worldWaterBtn = wrapBox[0].getElementsByClassName(
      "text-base text-yellow-400 font-normal"
    )[0];
  }
  if (wrapBox && wrapBox[1]) {
    sunWaterBtn = wrapBox[1].getElementsByClassName(
      "text-base text-yellow-400 font-normal"
    )[0];
  }

  if (wrapBox && wrapBox[1]) {
    moonWarterBtn = wrapBox[2].getElementsByClassName(
      "text-base text-yellow-400 font-normal"
    )[0];
  }

  if (_worldWaterBtn) {
    var _worldWarter = _worldWaterBtn.textContent
      .replace("\n", "")
      .replace("\n", "")
      .replace("/ 15", "")
      .trim();
    localStorage.setItem(ww, _worldWarter);
  }
  if (sunWaterBtn) {
    var sunWarter = sunWaterBtn.textContent
      .replace("\n", "")
      .replace("\n", "")
      .replace("/ 15", "")
      .trim();
    localStorage.setItem(sn, sunWarter);
  }
  if (moonWarterBtn) {
    var moonWarter = moonWarterBtn.textContent
      .replace("\n", "")
      .replace("\n", "")
      .replace("/ 15", "")
      .trim();
    localStorage.setItem(mn, moonWarter);
  }

  // check redeem
  // world
  if (location.href == farmLink) {
    if (wrapBox && wrapBox[0]) {
      const worldRedeemBtn = wrapBox[0].getElementsByClassName(
        "font-bold py-2 px-4 border-b-4 rounded"
      );
      if (
        worldRedeemBtn &&
        worldRedeemBtn[0] &&
        worldRedeemBtn[0].textContent.includes("Already rewarded")
      ) {
        localStorage.setItem(ww, "Already rewarded");
      }
      if (
        worldRedeemBtn &&
        worldRedeemBtn[0] &&
        worldRedeemBtn[0].textContent.includes("Redeem reward")
      ) {
        worldRedeemBtn[0].click();
        setInterval(() => {
          location.reload();
        }, 5000);
        return;
    ...
    // sun
    if (wrapBox && wrapBox[1]) {
      const sunRedeemBtn = wrapBox[1].getElementsByClassName(
        "font-bold py-2 px-4 border-b-4 rounded"
      );
      if (
        sunRedeemBtn &&
        sunRedeemBtn[0] &&
        sunRedeemBtn[0].textContent.includes("Already rewarded")
      ) {
        localStorage.setItem(sn, "Already rewarded");
      }
      if (
        sunRedeemBtn &&
        sunRedeemBtn[0] &&
        sunRedeemBtn[0].textContent.includes("Redeem reward")
      ) {
        sunRedeemBtn[0].click();
        setInterval(() => {
          location.reload();
        }, 5000);
        return;
    ...
    // moon
    if (wrapBox && wrapBox[2]) {
      const moonRedeemBtn = wrapBox[2].getElementsByClassName(
        "font-bold py-2 px-4 border-b-4 rounded"
      );
      if (
        moonRedeemBtn &&
        moonRedeemBtn[0] &&
        moonRedeemBtn[0].textContent.includes("Already rewarded")
      ) {
        localStorage.setItem(mn, "Already rewarded");
      }
      if (
        moonRedeemBtn &&
        moonRedeemBtn[0] &&
        moonRedeemBtn[0].textContent.includes("Redeem reward")
      ) {
        moonRedeemBtn[0].click();
        setInterval(() => {
          location.reload();
        }, 5000);
        return;
  ...
  function play(curMapLink, saveKey, worldWarterNum, mapObject, resultKey) {
    log("Play: ");
    ...
    if (location.href.replace(curMapLink, "")?.length > 1) {
      ...
      // detail page
      var foundBtn;
      const plantItems = document.getElementsByClassName(
        "bg-blue-600 hover:bg-blue-400 w-full text-white font-bold py-2 px-4 border-b-4 border-blue-800 hover:border-blue-600 rounded"
      );
      Array.from(plantItems).forEach(function (element, index, array) {
        if (element.textContent.includes("Interact")) {
          foundBtn = element;
        }
      });
      if (foundBtn) {
        log("Click pot"+ resultKey+  parseInt(worldWarterNum) + 1);
        localStorage.setItem(resultKey, parseInt(worldWarterNum) + 1);
        foundBtn.click();
  ...
  setTimeout(() => {
    var wwData = parseInt(localStorage.getItem(ww));
    var snData = parseInt(localStorage.getItem(sn));
    var mnData = parseInt(localStorage.getItem(mn));
    log("World: " + localStorage.getItem(ww));
    log("Sun: " + localStorage.getItem(sn));
    log("Moon: " + localStorage.getItem(mn));
    if (wwData >= 0 || snData >= 0 || mnData >= 0) {
      log("Has >=0");
      if (wwData != NaN && !(wwData + "").includes("15") && localStorage.getItem(ww) != 'Already rewarded') {
        log("world");
        play(worldMapLink, "worlds", wwData, worldMapList, ww);
      } else if (snData != NaN && !(snData + "").includes("15") && localStorage.getItem(sn) != 'Already rewarded') {
        log("suns");
        play(sunMapLink, "suns", snData, sunMapList, sn);
      } else if (mnData != NaN && !(mnData + "").includes("15") && localStorage.getItem(mn) != 'Already rewarded') {
        log("moons");
        play(moonMapLink, "moons", mnData, moonMapList, mn);
      } else {
        if (
          !localStorage.getItem(ww).includes("Already rewarded") ||
          !localStorage.getItem(sn).includes("Already rewarded") ||
          !localStorage.getItem(mn).includes("Already rewarded")
        ) {
      ...
      if (
        !localStorage.getItem(ww) ||
        !localStorage.getItem(ww).includes("Already rewarded") ||
        !localStorage.getItem(sn) ||
        !localStorage.getItem(sn).includes("Already rewarded") ||
